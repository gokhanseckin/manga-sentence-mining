...
struct ReviewView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(loc.t("review.title"))
...
private struct QuestionScreen: View {
        ...
        VStack(spacing: 20) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    if let translation = cloze.sentence?.translation,
    ...
    private var counterRow: some View {
        HStack {
            Text(loc.t("review.remaining", String(session.remainingUniqueCount)))
                .font(.subheadline)
                .foregroundStyle(.secondary)
            Spacer()
            Toggle(isOn: Binding(
                ...
                Label(loc.t("review.knownToggle"), systemImage: "checkmark.seal")
            ...
            Button {
                ...
                Label(
                    session.furiganaShown ? loc.t("furigana.hide") : loc.t("furigana.show"),
...
private struct AllCaughtUp: View {
    ...
    var body: some View {
        ContentUnavailableView(
            loc.t("review.allCaughtUp.title"),
            systemImage: "checkmark.seal",
            description: Text(loc.t("review.allCaughtUp.body"))
        )
    }
}
...
private struct SessionDone: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text(loc.t("review.completed")).font(.title3.bold())
            Button(loc.t("common.done"), action: onClose).buttonStyle(.borderedProminent)
        }
        .padding()
    }
}
